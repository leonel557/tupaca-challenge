...
import { FeedFilters, ArticlesFilters } from '../../../types/article';
...
function buildTabsNames(selectedTab: string) {
  ...
  return Array.from(new Set([...(user.isSome() ? ['Your Feed'] : []), 'Global Feed', 'Marcas', selectedTab]));
}
...
async function getFeedOrGlobalArticles(filters: FeedFilters = {}) {
  ...
  if (selectedTab.startsWith('#')) {
    finalFilters.tag = selectedTab.slice(2);
  }
  if (selectedTab === 'Marcas') {
    const marcas = ['Tupaca', 'Toyota', 'Apple', 'Amazon', 'Microsoft', 'Google', 'Ford'];
    finalFilters.marcas = marcas;
  }

  return await (selectedTab === 'Your Feed' ? getFeed : getArticles)(finalFilters);
}
